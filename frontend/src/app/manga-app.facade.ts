...
export class MangaAppFacade {
  ...
  // Called by App when auth flips to unauthenticated so the next login starts
  // from a clean slate. Kept as an explicit method so the auth/data coupling
  // is visible at the call site rather than buried in an effect.
  clearForLogout(): void {
    this.mangaList.set([]);
